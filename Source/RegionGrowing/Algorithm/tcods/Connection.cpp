...
namespace tcods
{
   ...
   // build the system of constraint equations
   {
      ...
      cm::Sparse A( common, mesh.edges.size(), basisCycles.size() );
      ...
      K = cm::Dense( common, basisCycles.size(), 1 );
      b = cm::Dense( common, basisCycles.size(), 1 );
   ...
   void Connection :: applyCotanWeights( cm::Sparse& A )
   {
      for(cm::Sparse::iterator e = A.begin(); e != A.end(); e++ )
      {
   ...
   void Connection :: buildCycleMatrix( cm::Sparse& A, vector<Mesh::Cycle>& cycles ) const
   {
