...
private:
	...
	*/ 
	...
	auto _cubicPatch_copyToOutput( CubicPatch& patch, Eigen::Vector3f targetCentroid ) -> std::vector<FElement*>
	{
		std::vector<FElement*> result;

		Eigen::Vector3f offset = targetCentroid - patch.centroid;

		for(FElement * exampleElement : patch.elements)
		{
			Eigen::Vector3f newPosition = exampleElement->position + offset;

			//if( _overlaps( newPosition, exampleElement->radius * relaxation ) )
			//	continue;

			FElement * newElement = _copyExemplarToOutput( exampleElement, newPosition, _defaultSelection );
	...
	auto _updateExemplarLimits() -> void
	{
		...
		for( FElement * element : this->_defaultSelection->_generativeElements)
		{
			Eigen::Vector3f elementHalfExtents( element->radius, element->radius, element->radius );

			Eigen::AlignedBox3f elementBox( element->position - elementHalfExtents, element->position + elementHalfExtents );
