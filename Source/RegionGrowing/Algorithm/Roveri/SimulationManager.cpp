...
SimulationManager::SimulationManager()
{
	...
	//-----------------------------------------------------------------------------------------------------------------------
	 ...
	 scene = 2; // nothing -1, liana 0, details on branch 1, 3d chair 2, features wall 3, ivy 10 , carnival 11, smoke 12, DTE 13, text synt 14, pallini 15, waves 16, bone 17
}
...
{
	omega = inputCloud.target_average_distance;
	...
	// Check if 2D
	if(inputCloud.cloud.samples[0].position[2] < 0.01 && inputCloud.cloud.samples[1].position[2] < 0.01) {
		is_2d = 1;
	}
...
void SimulationManager::run()
{
	...
	if (allow_add_points || allow_delete_points){
		...
		if (allow_delete_points){
			if (outputCloud.cloud.samples.size() > 100){
				deleteBadSamples();
			}
		}
...
void SimulationManager::addStep()
{
	...
	for (int ii=0; ii<myIndices.size(); ii++){
		...
		add_dir = add_dir.normalized() * inputCloud.average_samples_distance * output_estimated_scales[i];
		...
		if (scene == 0 || scene == 10 || scene == 13 || SGPScene == 1){
			...
			attributes.push_back(new_normal);
			attributes.push_back(outputCloud.cloud.samples[i].attributes[1]);
