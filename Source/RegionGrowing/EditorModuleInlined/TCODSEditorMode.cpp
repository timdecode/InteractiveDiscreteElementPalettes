...
bool FTCODSEditorMode::CapturedMouseMove( FEditorViewportClient* InViewportClient, FViewport* InViewport, int32 InMouseX, int32 InMouseY )
{
    ...
    if( InViewportClient->IsPerspective() && InViewportClient->EngineShowFlags.ModeWidgets )
    {
		...
		else if(_isDragging && _clickMode == ClickMode::SketchVoid)
			return _voidUpdate( InViewportClient, InViewport );
...
bool FTCODSEditorMode::InputKey( FEditorViewportClient* InViewportClient, FViewport* InViewport, FKey InKey, EInputEvent InEvent )
{
	...
	else if( _clickMode == ClickMode::Stretch || _clickMode == ClickMode::SketchVoid )
    {
...
bool FTCODSEditorMode::LeftMouseDown( FEditorViewportClient* InViewportClient, FViewport* InViewport, FKey InKey )
{
    ...
    if(InViewportClient->IsPerspective() && InViewportClient->EngineShowFlags.ModeWidgets)
    {
		...
		else if(!isShiftDown && _clickMode == ClickMode::SketchVoid)
		{
			_voidClick( InViewportClient, InViewport, staticMesh, regionGrower );
			return true;
		}
    }
...
void FTCODSEditorMode::_selectRegionGrower( FEditorViewportClient* InViewportClient, FViewport* InViewport )
{
	...
	if(_regionGrower && exampleSelectionHandle)
		_regionGrower->removeExampleSelection( exampleSelectionHandle );
...
URegionGrowingComponent* FTCODSEditorMode::_hitRegionGrower( FEditorViewportClient* InViewportClient, FViewport* InViewport )
{
	...
	for(TObjectIterator<URegionGrowingComponent> itr; itr; ++itr)
	{
		...
		FVector hitPosition = std::get<1>( hit );
...
bool FTCODSEditorMode::LeftMouseUp( FEditorViewportClient* InViewportClient, FViewport* InViewport, FKey InKey )
{
    if(_isDragging && _clickMode == ClickMode::Stretch)
		...
		return _voidEnd( InViewportClient, InViewport );
...
void FTCODSEditorMode::_strechClick( FEditorViewportClient* InViewportClient, FViewport* InViewport, UStaticMeshComponent * staticMesh, URegionGrowingComponent * regionGrower)
{
	...
	FVector hitPoint = std::get<1>( hit );
	uint32 face = std::get<2>( hit );
...
bool FTCODSEditorMode::_sketchUpdate( FEditorViewportClient* InViewportClient, FViewport* InViewport )
{
    ...
    _regionGrower->addBrushPoint( hitPoint, face );
...
bool FTCODSEditorMode::_sketchSelectUpdate( FEditorViewportClient* InViewportClient, FViewport* InViewport )
{
	...
	world->OverlapMultiByObjectType( overlaps, hitPoint, FQuat::Identity, queryParams, collisionShape );
...
void FTCODSEditorMode::_updateSelection()
{
	...
	if(!exampleSelectionHandle)
		_regionGrower->addExampleSelection( actorsVector, 1.0f );
	else
		_regionGrower->updateExampleSelection( exampleSelectionHandle, actorsVector, 1.0f );
}
...
bool FTCODSEditorMode::_eraseUpdate( FEditorViewportClient* InViewportClient, FViewport* InViewport )
{
	...
	_regionGrower->eraseAt( hitPoint, face );
...
void FTCODSEditorMode::_voidClick( FEditorViewportClient* InViewportClient, FViewport* InViewport, UStaticMeshComponent * staticMesh, URegionGrowingComponent * regionGrower )
{
	_regionGrower = regionGrower;

	_isDragging = true;

	_regionGrower->startVoid();
}

bool FTCODSEditorMode::_voidUpdate( FEditorViewportClient* InViewportClient, FViewport* InViewport )
{
	if(!_regionGrower)
		return false;
	...
	_regionGrower->addVoid( hitPoint, face );

	return true;
}

bool FTCODSEditorMode::_voidEnd( FEditorViewportClient* InViewportClient, FViewport* InViewport )
{
	_isDragging = false;

	_regionGrower->endVoid();
        ...
        [
            ...
            [
                ...
                [
                    SNew(SCheckBox)
                    .Style(FEditorStyle::Get(), "RadioButton")
                    .IsChecked( this, &STrivialConnectionWidget::IsRadioChecked, FTCODSEditorMode::ClickMode::SketchVoid )
                    .OnCheckStateChanged_Lambda([this](ECheckBoxState newRadioState)->void
                    {
                        this->editorMode->setClickMode(FTCODSEditorMode::ClickMode::SketchVoid);
                    })
                    .Content()
                    [
                        SNew(STextBlock).Text(NSLOCTEXT("STrivialConnectionWidget", "Void", "Void"))
                    ]
                ]
            ]
            +SVerticalBox::Slot()
